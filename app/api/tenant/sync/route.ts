...
async function handleSync(req: NextRequest) {
  ...
  const partsQ = sinceQuery(
    tenantQuery(db.from('parts').select('*'), tid).order('name'),
    since,
  )
  const customersQ = sinceQuery(
    tenantQuery(db.from('customers').select('*'), tid).order('full_name'),
    since,
  )
  ...
  const [
    ...
    incremental
      ...
      : tenantQuery(db.from('purchases').select('*'), tid).order('created_at', { ascending: false }).limit(200),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('todos').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('stolen_imeis').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('customer_orders').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('store_products').select('*'), tid).order('name'),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('assets').select('*'), tid).order('name'),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('campaigns').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('deals').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('showcase_devices').select('*'), tid).order('created_at', { ascending: false }),
    incremental
      ? Promise.resolve({ data: [], error: null })
      : tenantQuery(db.from('branches').select('*'), tid).order('name'),
    db.from('tenants').select('company_name, phone, address, shop_name, shop_logo, portal_slug').eq('id', tid).maybeSingle(),
    tenantQuery(db.from('tenant_settings').select('settings, updated_at'), tid).maybeSingle(),
    tenantQuery(db.from('accounts').select('balance'), tid).eq('type', 'kasa').limit(1),
  ])

  // Service orders with graceful join fallback
  let ordersRes = await sinceQuery(
    tenantQuery(
      db.from('service_orders').select('*, technician:user_profiles!technician_id(full_name)'),
      tid,
    ).order('created_at', { ascending: false }),
    since,
  ).limit(300)
  ...
  if (ordersRes.error) {
    ordersRes = await sinceQuery(
      tenantQuery(db.from('service_orders').select('*'), tid).order('created_at', { ascending: false }),
      since,
    ).limit(300)
  }
  ...
  const [
    ...
    : await Promise.all([
    tenantQuery(db.from('appointments').select('*'), tid).order('appointment_date', { ascending: false }),
    tenantQuery(db.from('warranties').select('*'), tid).order('created_at', { ascending: false }),
    tenantQuery(db.from('invoices').select('*'), tid).order('created_at', { ascending: false }),
    tenantQuery(db.from('notification_logs').select('*'), tid).order('created_at', { ascending: false }).limit(200),
    tenantQuery(db.from('support_tickets').select('*'), tid).order('created_at', { ascending: false }),
    tenantQuery(db.from('cash_shifts').select('*'), tid).order('opened_at', { ascending: false }),
    tenantQuery(db.from('supplier_orders').select('*'), tid).order('created_at', { ascending: false }),
    tenantQuery(db.from('personnel_profiles').select('*'), tid).order('full_name'),
    tenantQuery(db.from('foreign_devices').select('*'), tid).order('created_at', { ascending: false }),
  ])
  ...
  if (orderIds.length > 0 && !incremental) {
    const statusHistoryRes = await db
...
export const GET = withApiHandler(handleSync, 'tenant/sync')
export const POST = withApiHandler(handleSync, 'tenant/sync')
