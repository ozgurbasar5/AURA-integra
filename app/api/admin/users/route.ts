...
export async function POST(request: NextRequest) {
  ...
  let body: { tenant_id?: string; email?: string; password?: string; role?: string; full_name?: string }
  try {
  ...
  const { tenant_id, email, password, role, full_name } = body
  if (!tenant_id || !email || !password) {
    return NextResponse.json({ error: 'tenant_id, email, password gerekli' }, { status: 400 })
  }
