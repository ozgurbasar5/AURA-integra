...
import {
  getTransactions, getFinanceSummary, onStoreChange, isCariTransaction,
  upsertFinanceTransaction,
  removeFinanceTransactionById,
  type FinanceTransaction
} from '@/lib/store'
import { createFinanceTransactionAction } from '@/app/actions/finance'
import TransactionRow from '@/components/finans/TransactionRow'
...
export default function FinansPage() {
  ...
  const handleSave = async () => {
    ...
    upsertFinanceTransaction(optimistic)
    setTransactions(getTransactions())
    setSummary(getFinanceSummary())
    try {
      ...
      toast.success(`${modalType === 'gelir' ? 'Gelir' : 'Gider'} kaydedildi — ${formatCurrency(amount)}`)
      setShowModal(false)
      setFormData({ description: '', category: '', amount: '', payment_method: 'nakit', date: '', note: '' })
      refresh()
    } catch (e) {
      removeFinanceTransactionById(optimistic.id)
      refresh()
